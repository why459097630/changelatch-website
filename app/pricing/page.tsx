...
import { ArrowLeft, ArrowRight, CheckCircle2, Download, Info } from "lucide-react";
...
const RENEW_OPTIONS = [
  { id: "personal", label: "Personal License", priceLabel: "$29.99", days: 0 },
] as const;

type RenewId = "personal";
  ...
  const [selectedRenewId, setSelectedRenewId] = useState<RenewId>("personal");
...
useEffect(() => {
    ...
    const capturePayPalRenewal = async () => {
      try {
        ...
        if (window.sessionStorage.getItem(captureKey) === "1") {
          const fallbackRenewId = selectedRenewId;
        ...
        const finalRenewId: RenewId = selectedRenewId;
  ...
  const normalizedLicenseEmail = licenseEmail.trim().toLowerCase();
  const isLicenseEmailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedLicenseEmail);
                ...
                <div className="relative overflow-hidden rounded-[24px] border border-slate-200/70 bg-[linear-gradient(135deg,rgba(255,255,255,0.92),rgba(248,250,252,0.84)_42%,rgba(239,246,255,0.72)_100%)] px-4 py-5 shadow-[0_24px_70px_rgba(15,23,42,0.08),0_0_46px_rgba(99,102,241,0.10)] backdrop-blur-xl md:rounded-[28px] md:px-6 md:py-6">
                  ...
                  <div className="relative z-10">
                    <div className="space-y-5">
                      <div>
                        <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-fuchsia-600">
                          01
                        </div>
                      ...
                      <div>
                        ...
                        <div className="mt-2 text-[13px] leading-6 text-slate-500">
                          Send the prompt and ZIP package to web AI to generate precise code changes.
                        </div>
                      </div>
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="mt-4 space-y-2.5">
                        <div className="flex items-center gap-2.5 text-[13px] font-medium text-slate-700">
                          ...
                          <span>No license key required</span>
                        </div>

                        <div className="flex items-center gap-2.5 text-[13px] font-medium text-slate-700">
                          <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" />
                          <span>Try the core PATCH workflow</span>
                        </div>

                        <div className="flex items-center gap-2.5 text-[13px] font-medium text-slate-700">
                          <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" />
                          <span>Purchase a license anytime</span>
                        </div>
                      </div>
                ...
                <div className="order-1 md:order-2">
                  ...
                  <div className="grid grid-cols-2 gap-2.5 sm:gap-3">
                    <div className="flex items-center gap-2 rounded-[18px] border border-slate-200/70 bg-white/80 px-3 py-3 shadow-[0_12px_28px_rgba(15,23,42,0.04)] backdrop-blur-xl md:gap-3 md:rounded-[22px] md:px-4 md:py-4">
                      <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" />
                      <div>
                        <div className="text-sm font-semibold text-[#0f172a]">Multi-file changes</div>
                        <div className="mt-0.5 text-xs leading-5 text-slate-500">Apply one PATCH across multiple project files.</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 rounded-[18px] border border-slate-200/70 bg-white/80 px-3 py-3 shadow-[0_12px_28px_rgba(15,23,42,0.04)] backdrop-blur-xl md:gap-3 md:rounded-[22px] md:px-4 md:py-4">
                      <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" />
                      <div>
                        <div className="text-sm font-semibold text-[#0f172a]">Exact matching</div>
                        <div className="mt-0.5 text-xs leading-5 text-slate-500">Apply changes only when the original code matches exactly.</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 rounded-[18px] border border-slate-200/70 bg-white/80 px-3 py-3 shadow-[0_12px_28px_rgba(15,23,42,0.04)] backdrop-blur-xl md:gap-3 md:rounded-[22px] md:px-4 md:py-4">
                      <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" />
                      <div>
                        <div className="text-sm font-semibold text-[#0f172a]">Automatic backups</div>
                        <div className="mt-0.5 text-xs leading-5 text-slate-500">Create a backup before applying each PATCH.</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 rounded-[18px] border border-slate-200/70 bg-white/80 px-3 py-3 shadow-[0_12px_28px_rgba(15,23,42,0.04)] backdrop-blur-xl md:gap-3 md:rounded-[22px] md:px-4 md:py-4">
                      <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" />
                      <div>
                        <div className="text-sm font-semibold text-[#0f172a]">Build verification</div>
                        <div className="mt-0.5 text-xs leading-5 text-slate-500">Automatically run the project build after applying changes.</div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-5 px-5 py-5 md:px-6 md:py-6">
              <div className="grid gap-3">
                {RENEW_OPTIONS.map((option) => {
                  ...
                  const isBestValue = option.id === "personal";
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <div className="mt-2 space-y-1 text-[12px] leading-4 text-slate-500">
                            ...
                            <div>Activate on up to 2 devices</div>
                          </div>
                        </div>
              ...
              <div>
                <label
                  ...
                  className="block text-[12px] font-semibold text-[#0f172a]"
                >
                  License email
                </label>
                <input
              ...
              <button
                type="button"
                disabled={isSubmitting || !isLicenseEmailValid}
                onClick={async () => {
                  if (!isLicenseEmailValid) {
                    setSubmitError("Enter a valid license email.");
                    return;
                  }
                  ...
                  try {
                    ...
                    const response = await fetch("/api/paypal/create-license-order", {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
                      },
                      body: JSON.stringify({
                        licenseEmail: normalizedLicenseEmail,
                      }),
                    });
                    ...
                    if (!response.ok || !data?.ok || !data?.url) {
                      throw new Error(data?.error || "Failed to create PayPal license order.");
                    }
              ...
              >
                ...
                <div className="relative flex items-center justify-center gap-2">
                  <span className="text-[15px]">
                    {isSubmitting ? "Redirecting to PayPal..." : "Purchase License"}
                  </span>
              ...
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-fuchsia-600">
                  After purchase
                </div>
                ...
                <div className="mt-3 space-y-2.5">
                  <div className="flex items-start gap-2.5 text-[12px] leading-5 text-slate-600">
                    ...
                    <span>Your License Key will be generated immediately</span>
                  </div>

                  <div className="flex items-start gap-2.5 text-[12px] leading-5 text-slate-600">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-500" />
                    <span>It will be linked to your license email</span>
                  </div>
              ...
              <div className="space-y-3 break-words text-center text-[12px] leading-5 text-slate-500">
                ...
                <div>
                  License purchases are non-refundable after activation.
                </div>
              </div>
